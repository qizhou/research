// evm_warm_read.c
// Build:  gcc -O2 -std=c11 -pthread evm_warm_read.c -o evm_warm_read
// Run examples:
//   ./evm_warm_read               # defaults (4096 pages, enough to thrash most STLBs)
//   ./evm_warm_read  256          # ~1MB working set, often fits in STLB
//   ./evm_warm_read  4096  10     # 4096 pages, 10 passes
//   sudo taskset -c 1 ./evm_warm_read 4096 10 # pin to CPU 1 (more stable)
...
int main(int argc, char *argv[]) {
    ...
    volatile uint64_t sink = 0; // use sink to avoid compiler optimization.
    ...
    // Results
    double total_accesses = (double)bytes * (double)passes / 32;
