...
func loadDynamic() {
	...
	fmt.Println("loading frac.o")
	now = time.Now()
	engine.AddObjectFileByFilename("frac.o")
	fmt.Printf("loaded used time %d ns\n", time.Now().Sub(now).Nanoseconds())

	pointer = engine.GetFunctionAddress("frac")
	fmt.Println("fib func pointer", pointer)
	...
	res = new(big.Int).SetBytes(reverse(buf[:]))
	fmt.Printf("llvm getFunctionAddress(): frac(%d) = %s, used time %d ns\n", input, res.String(), time.Now().Sub(now).Nanoseconds())
}
...
func compileAndRunFrac() {
	...
	fib_func := llvm.AddFunction(module, "frac", fib_type)
	...
	builder.CreateStore(llvm.ConstInt(uint_type, 1, false), a_ptr)
	builder.CreateStore(llvm.ConstInt(ctx.Int32Type(), 2, false), i_ptr)
	builder.CreateBr(loop_block)

	builder.SetInsertPointAtEnd(loop_block)
	i_val := builder.CreateLoad(ctx.Int32Type(), i_ptr, "i_val")
	cond := builder.CreateICmp(llvm.IntULE, i_val, n, "loopcond")
	...
	i_ext_val := builder.CreateZExt(i_val, uint_type, "i_ext_val")
	next_a := builder.CreateMul(a_val, i_ext_val, "next_a")
	...
	os.WriteFile("frac.ll", []byte(module.String()), 0644)
	...
	os.WriteFile("frac.o", mem.Bytes(), 0644)

	mem, err = targetMachine.EmitToMemoryBuffer(module, llvm.AssemblyFile)
	if err != nil {
		panic(err)
	}
	os.WriteFile("frac.asm", mem.Bytes(), 0644)

	file, err := os.OpenFile("frac.bc", os.O_CREATE|os.O_RDWR, 0644)
	if err != nil {
		panic(err)
	}
